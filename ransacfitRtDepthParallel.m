...
function [Rt, inliers] = ransacfitRtDepthParallel(x, t, feedback, XYZcam_i, XYZcam_j, K)
    s = 3;  % Number of points needed to fit a Rt matrix.

    fittingfn = @estimateRt;
    distfn    = @euc3DdistDepthParallel;
    degenfn   = @isdegenerate;
    % x1 and x2 are 'stacked' to create a 6xN array for ransac
    [Rt, inliers] = ransacGeneralParallel(x, fittingfn, distfn, degenfn, s, t, feedback, XYZcam_i, XYZcam_j, K);
...
function [bestInliers, bestRt, bestNinliers] = euc3DdistDepthParallel(Rt, x, t, XYZcam_i, XYZcam_j, K)
    if iscell(Rt)  % We have several solutions each of which must be tested
        ...
        if bestNinliers>3 %size(x,2)*0.30
            bestNinliers = bestNinliers * alignmentQualityParallel(Rt, XYZcam_i, XYZcam_j, K);
            if isnan(bestNinliers) || bestNinliers<0
                bestNinliers=1;
            end
        end
